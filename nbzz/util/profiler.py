...
# the output will be printed to your nbzz root path, e.g. ~/.nbzz/mainnet/profile/
# to analyze the profile, run:

#   python nbzz/utils/profiler.py ~/.nbzz/mainnet/profile | less -r

# this will print CPU usage of the nbzz full node main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python nbzz/utils/profiler.py ~/.nbzz/mainnet/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int):
        ...
        output_file = "nbzz-hotspot-%d" % first
        if first < last:
            output_file += "-%d" % last
