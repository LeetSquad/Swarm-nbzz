...
from nbzz.cmds.init_funcs import nbzz_init
from nbzz.daemon.windows_signal import kill
from nbzz.server.server import ssl_context_for_root, ssl_context_for_server
from nbzz.ssl.create_ssl import get_mozilla_ca_crt
from nbzz.util.nbzz_logging import initialize_logging
from nbzz.util.config import load_config
from nbzz.util.json_util import dict_to_json_str
from nbzz.util.path import mkdir
from nbzz.util.service_groups import validate_service
from nbzz.util.setproctitle import setproctitle
from nbzz.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting nbzz.")
    quit()
...
service_plotter = "nbzz plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "nbzz": "nbzz",
        "nbzz_wallet": "start_wallet",
        "nbzz_full_node": "start_full_node",
        "nbzz_harvester": "start_harvester",
        "nbzz_farmer": "start_farmer",
        "nbzz_introducer": "start_introducer",
        "nbzz_timelord": "start_timelord",
        "nbzz_timelord_launcher": "timelord_launcher",
        "nbzz_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("nbzz daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered nbzz_ROOT
    os.environ["nbzz_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with nbzz_ROOT: {os.environ['nbzz_ROOT']}")
