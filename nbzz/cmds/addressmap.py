...
@click.command("addressmap", short_help="set alias")
...
@click.pass_context
def addressmap_cmd(ctx: click.Context, password,bee_key_path,address) -> None:
    ...
    construct_txn = nbzz_contract.functions.setAddressMapping(address).buildTransaction({"nonce":w3.eth.getTransactionCount(my_local_acc.address),"gas":290_0000}) #address
    print(construct_txn)
    ...
    if tx_receipt["status"] !=1:
        print( "set address map fail")
    else:
        print( "set address map success")