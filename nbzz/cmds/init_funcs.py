...
#from nbzz.consensus.coinbase import create_puzzlehash_for_pk
from nbzz.ssl.create_ssl import generate_ca_signed_cert, get_nbzz_ca_crt_key, make_ca_cert
from nbzz.util.bech32m import encode_puzzle_hash
from nbzz.util.config import (
    create_default_nbzz_config,
    initial_config_file,
    load_config,
    save_config,
...
from nbzz.util.keychain import Keychain
from nbzz.util.path import mkdir
#from nbzz.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'nbzz keys generate'")
        return None
...
def create_all_ssl(root: Path):
    ...
    nbzz_ca_crt, nbzz_ca_key = get_nbzz_ca_crt_key()
    nbzz_ca_crt_path = ca_dir / "nbzz_ca.crt"
    nbzz_ca_key_path = ca_dir / "nbzz_ca.key"
    nbzz_ca_crt_path.write_bytes(nbzz_ca_crt)
    nbzz_ca_key_path.write_bytes(nbzz_ca_key)
    ...
    nbzz_ca_crt, nbzz_ca_key = get_nbzz_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, nbzz_ca_crt, nbzz_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if nbzz_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return nbzz_init(root_path)


def nbzz_version_number() -> Tuple[str, str, str, str]:
...
def nbzz_minor_release_number():
    res = int(nbzz_version_number()[2])
    print(f"Install release number: {res}")
    return res


def nbzz_full_version_str() -> str:
    major, minor, patch, dev = nbzz_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def nbzz_init(root_path: Path):
    if os.environ.get("NBZZ_ROOT", None) is not None:
        print(
            f"warning, your NBZZ_ROOT is set to {os.environ['NBZZ_ROOT']}. "
            f"Please unset the environment variable and run nbzz init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Nbzz directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if NBZZ_ROOT is set, or if user has run nbzz init twice
        # before a new update.
        #check_keys(root_path)
        print(f"{root_path} already exists, no migration action taken")
        return -1

    create_default_nbzz_config(root_path)
    create_all_ssl(root_path)
    #check_keys(root_path)
    print("")
