...
from nbzz.protocols.protocol_message_types import ProtocolMessageTypes
from nbzz.protocols.shared_protocol import protocol_version
from nbzz.server.introducer_peers import IntroducerPeers
from nbzz.server.outbound_message import Message, NodeType
from nbzz.server.ssl_context import private_ssl_paths, public_ssl_paths
from nbzz.server.ws_connection import WSNbzzConnection
from nbzz.types.blockchain_format.sized_bytes import bytes32
from nbzz.types.peer_info import PeerInfo
from nbzz.util.errors import Err, ProtocolError
from nbzz.util.ints import uint16
from nbzz.util.network import is_localhost, is_in_network
...
class NbzzServer:
    def __init__(
        ...
        nbzz_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSNbzzConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSNbzzConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.nbzz_ca_crt_path, self.nbzz_ca_key_path = nbzz_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSNbzzConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.nbzz_ca_crt_path, self.nbzz_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSNbzzConnection] = None
        try:
            connection = WSNbzzConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSNbzzConnection, on_connect=None):
    ...
    def connection_closed(self, connection: WSNbzzConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSNbzzConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSNbzzConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSNbzzConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSNbzzConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSNbzzConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSNbzzConnection, trusted_peers: Dict) -> bool:
