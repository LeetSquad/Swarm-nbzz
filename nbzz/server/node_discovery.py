...
import nbzz.server.ws_connection as ws
import dns.asyncresolver
from nbzz.protocols import full_node_protocol, introducer_protocol
from nbzz.protocols.protocol_message_types import ProtocolMessageTypes
from nbzz.server.address_manager import AddressManager, ExtendedPeerInfo
from nbzz.server.address_manager_store import AddressManagerStore
from nbzz.server.outbound_message import NodeType, make_msg
from nbzz.server.server import NbzzServer
from nbzz.types.peer_info import PeerInfo, TimestampedPeerInfo
from nbzz.util.hash import std_hash
from nbzz.util.ints import uint64
from nbzz.util.path import mkdir, path_from_root
...
class FullNodeDiscovery:
    ...
    async def on_connect(self, peer: ws.WSNbzzConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSNbzzConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSNbzzConnection):
