...
from nbzz.daemon.server import singleton, service_launch_lock_path
from nbzz.server.ssl_context import nbzz_ssl_ca_paths, private_ssl_ca_paths
...
from nbzz.rpc.rpc_server import start_rpc_server
from nbzz.server.outbound_message import NodeType
from nbzz.server.server import NbzzServer
from nbzz.server.upnp import UPnP
from nbzz.types.peer_info import PeerInfo
from nbzz.util.nbzz_logging import initialize_logging
from nbzz.util.config import load_config, load_config_cli
...
class Service:
    def __init__(
        ...
        proctitle_name = f"nbzz_{service_name}"
        ...
        nbzz_ca_crt, nbzz_ca_key = nbzz_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = NbzzServer(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for NbzzServer to be closed")
