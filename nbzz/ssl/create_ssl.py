...
def get_nbzz_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "nbzz_ca.crt")
    key = pkg_resources.resource_string(__name__, "nbzz_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Nbzz"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Nbzz"),
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("nbzz.net")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Nbzz"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Nbzz CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Organic Farming Division"),
        ]
    )
    root_cert = (
...
def main():
    return make_ca_cert(Path("./nbzz_ca.crt"), Path("./nbzz_ca.key"))
